...
/**
 * ⚠️ ÉLES INCIDENS UTÁN SZÜLETETT (2026-08-04). A kezdőlap és a /szaknevsor
 * `force-dynamic`, és mindkettő a TELJES listát (2200+ rekord) húzta be, hogy
 * utána 3/ország, illetve 30/ország szeletet mutasson. A worker ettől
 * túllépte az erőforrás-korlátot: Cloudflare Error 1102, és a két
 * LEGFONTOSABB lap az esetek ~70%-ában 500/503-at adott.
 *
 * Ez a lekérdezés a SZELETELÉST AZ SQL-BE teszi (ablakfüggvény): a worker
 * legfeljebb `perCountry × 6` sort lát, nem 2200-at.
 *
 * ⚠️ A rendezés BETŰRE ugyanaz, mint a teljes listáé — különben a szelet más
 * cégeket adna, mint amit a felhasználó a teljes lista betöltése után lát
 * (a kliens a /api/businesses/list-ből tölti a teljeset), és a lista
 * „ugrálna" alatta.
 */
export async function getBusinessesForListSlice(
  perCountry: number,
  opts: { requireCoords?: boolean } = {},
): Promise<ListBusiness[]> {
  const n = Math.max(1, Math.min(200, Math.floor(perCountry)));
  // ⚠️ A kezdőlap CSAK koordinátás tételt tud kirakni a térkép-kártyára. Ha a
  // szűrést a szeletelés UTÁN végeznénk (ahogy korábban), a 3-as szeletből
  // koordináta nélküli sorok is elfogyaszthatnák a helyet, és az ország
  // kártyája üresen maradna. Ezért a feltétel a WHERE-be megy.
  const koordFeltetel = opts.requireCoords ? " AND lat IS NOT NULL AND lng IS NOT NULL" : "";
  return cached(`biz:list-slice-v1:${n}:${opts.requireCoords ? "geo" : "all"}`, LIST_TTL_MS, async () => {
    const { results } = await getDB()
      .prepare(
        `SELECT ${LIST_COLUMNS} FROM (
           SELECT ${LIST_COLUMNS},
                  ROW_NUMBER() OVER (
                    PARTITION BY COALESCE(country_code, 'CH')
                    ORDER BY featured DESC,
                             (CASE WHEN (phone IS NOT NULL AND trim(phone) <> '')
                                     OR (contact_email IS NOT NULL AND trim(contact_email) <> '')
                                     OR (blurb LIKE '% · %.%')
           ...
           FROM businesses
           WHERE COALESCE(hidden, 0) = 0 AND moderation_status = 1${koordFeltetel}
         ) WHERE rn <= ?`,
      )
      .bind(n)
      .all<ListBusinessRow>();
...
/**
 * Országonkénti darabszám a látható listából — a /szaknevsor azonnal kiírja,
 * amíg a teljes lista tölt. Korábban a teljes lista szeletelésének
 * mellékterméke volt; a szelet-lekérdezés mellé így külön, OLCSÓ skalár-query
 * kell (egy COUNT, nem 2200 sor).
 */
export async function getBusinessCountsByCountry(): Promise<Record<string, number>> {
  return cached("biz:counts-by-country-v1", LIST_TTL_MS, async () => {
    const { results } = await getDB()
      .prepare(
        `SELECT COALESCE(country_code, 'CH') AS cc, COUNT(*) AS n FROM businesses
         WHERE COALESCE(hidden, 0) = 0 AND moderation_status = 1
         GROUP BY cc`,
      )
      .all<{ cc: string; n: number }>();
    const out: Record<string, number> = {};
    for (const r of results) out[r.cc] = r.n;
    return out;
  });
}

/**
 * Országonként a legnépesebb kategóriák (SEO belső-link blokk). Korábban a
 * TELJES lista bejárásából jött; most GROUP BY, tehát pár tucat sor.
 */
export async function getTopCategoriesByCountry(limitPerCountry = 8): Promise<Record<string, { categoryId: string; n: number }[]>> {
  const lim = Math.max(1, Math.min(20, Math.floor(limitPerCountry)));
  return cached(`biz:topcats-v1:${lim}`, LIST_TTL_MS, async () => {
    const { results } = await getDB()
      .prepare(
        `SELECT cc, category_id, n FROM (
           SELECT COALESCE(country_code, 'CH') AS cc, category_id, COUNT(*) AS n,
                  ROW_NUMBER() OVER (PARTITION BY COALESCE(country_code, 'CH') ORDER BY COUNT(*) DESC) AS rn
           FROM businesses
           WHERE COALESCE(hidden, 0) = 0 AND moderation_status = 1
      ...
      .bind(lim)
      .all<{ cc: string; category_id: string; n: number }>();
    const out: Record<string, { categoryId: string; n: number }[]> = {};
    for (const r of results) (out[r.cc] ??= []).push({ categoryId: r.category_id, n: r.n });
    return out;
  });
}
