...
/**
 * Triggerezi az Albérlet radarokat, ha a kanton egyezik, vagy ha 'all' volt beállítva.
 * Mivel a Web Push jelenleg payload nélküli (csak ping), a kliens egy generikus
 * értesítést kap, ami bevonzva az appba megmutatja az új hirdetést.
 */
export async function triggerAlberletRadars(cantonCode: string) {
  if (!cantonCode) return;
  try {
    const radars = await getActiveRadarsByType("alberlet");
    ...
    // Szűrés a releváns radarokra
    const targets = radars.filter(r => {
      try {
        const p = JSON.parse(r.parameters);
        return p.canton === "all" || p.canton === cantonCode;
      } catch {
        return false;
      }
    });
    ...
    safeLogError("triggerAlberletRadars", err);
  }
}
