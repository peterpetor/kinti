...
export default function BenchmarkClient() {
  ...
  if (loading && locked) {
    return <div className="flex justify-center p-10"><div className="animate-spin w-8 h-8 border-4 border-brand-500 border-t-transparent rounded-full" /></div>;
  }
    ...
    <div className="bg-white dark:bg-neutral-900 shadow-xl rounded-2xl overflow-hidden border border-neutral-200 dark:border-neutral-800">
      ...
      <div className="flex border-b border-neutral-200 dark:border-neutral-800">
        <button 
          ...
          className={`flex-1 py-4 text-center font-semibold text-lg transition-colors ${activeTab === "salary" ? "text-brand-600 border-b-2 border-brand-600 dark:text-brand-400 dark:border-brand-400 bg-brand-50/50 dark:bg-brand-900/10" : "text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300"}`}
        >
          💰 Fizetések (Bérek)
        </button>
        <button 
          onClick={() => setActiveTab("rent")}
          className={`flex-1 py-4 text-center font-semibold text-lg transition-colors ${activeTab === "rent" ? "text-brand-600 border-b-2 border-brand-600 dark:text-brand-400 dark:border-brand-400 bg-brand-50/50 dark:bg-brand-900/10" : "text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300"}`}
        >
          🏠 Lakbérek (Albérlet)
        </button>
      </div>
        ...
        {locked ? (
          ...
          <div className="max-w-xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-xl border border-neutral-100 dark:border-neutral-800">
                ...
                <>
                  <div className="space-y-4">
                    <div>
                      ...
                      <select value={formCanton} onChange={e => setFormCanton(e.target.value)} className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 focus:ring-brand-500 focus:border-brand-500">
                    ...
                    <div>
                      ...
                      <select value={formIndustry} onChange={e => setFormIndustry(e.target.value)} className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 focus:ring-brand-500 focus:border-brand-500">
                        {INDUSTRIES.map(i => <option key={i} value={i}>{i}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Tapasztalat (Szakmában eltöltött évek)</label>
                      <input type="number" min="0" max="50" value={formExp} onChange={e => setFormExp(parseInt(e.target.value)||0)} className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 focus:ring-brand-500 focus:border-brand-500" />
                    </div>
                    <div>
                      ...
                      <input type="number" min="20000" max="300000" step="500" value={formSalary} onChange={e => setFormSalary(parseInt(e.target.value)||0)} required className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 text-lg font-semibold focus:ring-brand-500 focus:border-brand-500" />
                    </div>
                  </div>
                </>
              ) : (
                <>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select value={formRooms} onChange={e => setFormRooms(parseFloat(e.target.value))} className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 focus:ring-brand-500 focus:border-brand-500">
                    ...
                    <div>
                      ...
                      <input type="number" min="300" max="10000" step="50" value={formRent} onChange={e => setFormRent(parseInt(e.target.value)||0)} required className="w-full rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2.5 px-3 text-lg font-semibold focus:ring-brand-500 focus:border-brand-500" />
                    </div>
                  </div>
                </>
              )}
              ...
              <button disabled={submitting || !turnstileToken} type="submit" className="w-full flex items-center justify-center space-x-2 bg-brand-600 hover:bg-brand-700 text-white font-bold py-3 px-4 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {submitting ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-xl border border-neutral-100 dark:border-neutral-800">
              ...
              <select 
                ...
                className="w-full sm:w-64 rounded-lg border-neutral-300 bg-white dark:bg-neutral-900 dark:border-neutral-700 py-2 px-3 focus:ring-brand-500 focus:border-brand-500"
              >
            ...
            {loading && <div className="py-20 flex justify-center"><div className="animate-spin w-8 h-8 border-4 border-brand-500 border-t-transparent rounded-full" /></div>}
                ...
                {salaryStats.length === 0 ? (
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {salaryStats.map((stat, i) => (
                      <div key={i} className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-5 hover:border-brand-300 dark:hover:border-brand-700 transition-colors shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="font-bold text-lg">{stat.industry}</h3>
                          <span className="bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 text-xs font-bold px-2.5 py-1 rounded-full">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-bold text-brand-600 dark:text-brand-400 text-lg">{stat.avg_salary.toLocaleString("hu-HU")}</span>
                          </div>
                          <div className="w-full bg-neutral-100 dark:bg-neutral-800 rounded-full h-2.5 overflow-hidden">
                             ...
                             <div className="bg-brand-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, (stat.avg_salary / 150000) * 100)}%` }}></div>
                          </div>
                      ...
                      <div key={i} className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-5 hover:border-brand-300 dark:hover:border-brand-700 transition-colors shadow-sm">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-bold text-brand-600 dark:text-brand-400 text-lg">{stat.avg_rent.toLocaleString("hu-HU")}</span>
                          </div>
                          <div className="w-full bg-neutral-100 dark:bg-neutral-800 rounded-full h-2.5 overflow-hidden">
                             {/* Simple visual bar based on 4000 max */}
                             <div className="bg-brand-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, (stat.avg_rent / 4000) * 100)}%` }}></div>
                          </div>
                          <div className="flex justify-between text-xs text-neutral-400 pt-1">
