...
/**
 * Kétlépcsős törlés-vezérlő a véleményhez. Sikeres törlés után
 * visszairányít a szaknévsorra.
 */
export function ReviewManageActions({ token }: { token: string }) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/reviews/manage/${token}`, { method: "DELETE" });
      if (!res.ok) {
        const data = (await res.json().catch(() => ({}))) as { error?: string };
        throw new Error(data.error ?? "A törlés nem sikerült.");
      }
      setDone(true);
      setTimeout(() => router.push("/szaknevsor"), 1800);
    } catch (err) {
  ...
  if (done) {
    return (
      <div className="rounded-card border border-success/30 bg-surface p-4 text-center shadow-card">
        ...
        <p className="text-[11.5px] text-ink-muted">Visszairányítunk…</p>
      </div>
    );
  }
