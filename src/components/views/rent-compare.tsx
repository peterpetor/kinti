 ...
 * de kompakt, költség-fókuszú A/B összevetésre. Teljesen kliensoldali.
 */
interface FlatCfg {
  monthlyRent: number;
  size: FlatSize;
...
function chf(n: number) {
  return `${Math.round(n).toLocaleString("hu-HU")} CHF`;
}
...
export function RentCompare() {
  ...
  const ra = useMemo(
    () => calculateRentCost({ monthlyRentChf: a.monthlyRent, size: a.size, heating: a.heating, region: a.region, acontoNebenkostenChf: a.aconto, yearsToCalculate: years }),
    [a, years],
  );
  const rb = useMemo(
    () => calculateRentCost({ monthlyRentChf: b.monthlyRent, size: b.size, heating: b.heating, region: b.region, acontoNebenkostenChf: b.aconto, yearsToCalculate: years }),
    [b, years],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <FlatColumn tag="A" cfg={a} onChange={setA} highlight={cheaper === "A"} />
        <FlatColumn tag="B" cfg={b} onChange={setB} highlight={cheaper === "B"} />
      </div>
        ...
        {cheaper ? (
          <>
            ...
            <h2 className="mt-1 text-[20px] font-extrabold leading-tight text-ink">
              Lakás {cheaper} — évi {chf(annualDelta)}-fel kevesebb
            </h2>
            <p className="mt-1 text-[12px] leading-snug text-ink-muted">
              ≈ {chf(annualDelta * years)} különbség {years} év alatt (becsült teljes lakhatási költség alapján —
      ...
      <section className="overflow-hidden rounded-card border border-line bg-surface shadow-card">
        ...
        <CmpRow label="Havi fix (bér + akontó)" a={a.monthlyRent + a.aconto} b={b.monthlyRent + b.aconto} />
        <CmpRow label="Becsült NK / év" a={ra.estimatedActualNebenkostenPerYear} b={rb.estimatedActualNebenkostenPerYear} />
        <CmpRow label="Kaúció (lekötött pénz)" a={ra.kautionAmount} b={rb.kautionAmount} />
        <CmpRow label="Teljes 1. éves költség" a={totalA} b={totalB} strong />
        <CmpRow label={`Rejtett költség / ${years} év`} a={ra.totalHiddenCostOverPeriod} b={rb.totalHiddenCostOverPeriod} />
      </section>

      <p className="px-1 text-[11px] leading-snug text-ink-faint">
        A zölddel jelölt érték az olcsóbb az adott sorban. A becslés átlagos svájci adatokon alapul (32 CHF/m²/év
        NK-alapráta, 4% feltételezett hozam) — a tényleges költség eltérhet. Nem jogi vagy pénzügyi tanács.
      </p>
    </div>
  );
}
...
function FlatColumn({
  ...
  return (
    ...
    >
      ...
      <label className="block text-[10px] font-bold uppercase tracking-wide text-ink-faint">Havi bér (CHF)</label>
      <input
        type="number"
        min={0}
        max={20000}
      ...
      <select value={cfg.region} onChange={(e) => set({ region: e.target.value as Region })} className={selectCls}>
        {REGIONS.map((r) => (
          <option key={r.id} value={r.id}>{r.emoji} {r.label}</option>
        ))}
      </select>

      <label className="block text-[10px] font-bold uppercase tracking-wide text-ink-faint">Akontó NK (CHF/hó)</label>
      <input
        type="number"
        min={0}
        max={2000}
...
function CmpRow({ label, a, b, strong }: { label: string; a: number; b: number; strong?: boolean }) {
  const aBetter = a < b;
  const bBetter = b < a;
  return (
