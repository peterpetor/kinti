      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <div className="mt-0.5 text-[11.5px] text-ink-muted">
            Mutasd meg a munkáidat! Max. 10 kép, 2 MB/kép.
          </div>
        </div>
        <button
          type="button"
      ...
      {currentKeys.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {currentKeys.map((key) => {
            ...
            if (!url) return null;
            return (
              <div key={key} className="group relative h-20 w-20 overflow-hidden rounded-[12px] border border-line shadow-sm">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={url} alt="Galéria kép" className="h-full w-full object-cover" />
                <button
                  type="button"
                  disabled={busy}
        ...
        <div className="rounded-xl border border-dashed border-line bg-surface-alt px-4 py-6 text-center">
          ...
          <p className="text-[12.5px] font-medium text-ink-muted">Még nincsenek képek a galériádban.</p>
        </div>
      )}
