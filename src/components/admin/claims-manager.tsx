...
export function ClaimsManager({ initialClaims }: { initialClaims: BusinessClaim[] }) {
  ...
  return (
    <div className="mt-6 space-y-3">
      {msg && (
        <div className="rounded-card border border-primary/30 bg-primary-soft/40 px-4 py-2.5 text-[12.5px] font-semibold text-primary-ink">
          {msg}
        </div>
      )}
      {claims.map((c) => (
        <div key={c.id} className="rounded-card border border-line bg-surface p-4 shadow-card">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <Link href={`/szaknevsor/${c.businessId}`} target="_blank" className="text-[14.5px] font-extrabold text-ink hover:text-primary-ink">
