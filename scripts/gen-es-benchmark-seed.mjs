    ...
    for (const [yrs, em] of EXP) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        lines.push(
          ...
          `VALUES ('${id}', 'ES', '${code}', '${esc(industry)}', ${yExp}, ${gross}, '${ip}', datetime('now', '-${1 + (n % 200)} days'));`
        );
      }
    }
  }
...
for (const code of RG_CODES) {
  for (const rooms of ROOMS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const ip = `seed-es-rent-${m}`;
      m++;
      lines.push(
        `INSERT INTO rent_benchmarks (id, country_code, canton_code, rooms, rent_chf, ip_hash, created_at) ` +
        `VALUES ('${id}', 'ES', '${code}', ${rooms}, ${Math.max(250, rent)}, '${ip}', datetime('now', '-${1 + (m % 150)} days'));`
      );
    }
  }
}
